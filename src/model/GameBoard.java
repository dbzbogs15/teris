...
public class GameBoard implements Runnable, IModel {
    ...
    public void drawScore(Graphics g) {
        ...
        g2.setColor(new Color(66, 66, 66, 180));
        g2.drawRoundRect(20 + 10 * BLOCK_SIZE, 190, BLOCK_SIZE * 4, BLOCK_SIZE, 5, 5);
        g2.setFont(new Font("Digital-7", Font.BOLD, 30));
    ...
    public void drawHighScore(Graphics g) {
        ...
        g2.setColor(new Color(66, 66, 66, 180));
        g2.drawRoundRect(20 + 10 * BLOCK_SIZE, 255, BLOCK_SIZE * 4, BLOCK_SIZE, 5, 5);
        g2.setFont(new Font("Digital-7", Font.BOLD, 30));
        g2.setColor(Color.GREEN);
    ...
    public void drawLevel(Graphics g) {
        ...
        g2.setColor(new Color(66, 66, 66, 180));
        g2.drawRoundRect(20 + 10 * BLOCK_SIZE, 315, BLOCK_SIZE * 4, BLOCK_SIZE, 5, 5);
        g2.setFont(new Font("Digital-7", Font.BOLD, 30));
        g2.setColor(Color.GREEN);
        if (level == 1) {
    ...
    public void fixedBlockAndSetBlock() {
        clear();
        for (int i = 0; i < matrixBoard.length; i++) {
            System.arraycopy(matrixBoard[i], 0, tempMatrixBoard[i], 0, matrixBoard[i].length);
        }
        if (isLose()) {
            lose();
        }
    ...
    public boolean isExistOrOutArray(Position position) {
        ...
        return matrixBoard[position.getX()][position.getY()] != -1;
    }
    ...
    public void revertMatrix() {
        for (int i = 0; i < matrixBoard.length; i++) {
            System.arraycopy(tempMatrixBoard[i], 0, matrixBoard[i], 0, matrixBoard[i].length);
        }
    }
    ...
    public void delete(int line) {
        ...
        for (int i = 0; i < line; i++) {
            System.arraycopy(matrixBoard[i], 0, temp[i], 0, matrixBoard[i].length);
        }
        for (int i = 0; i < temp.length; i++) {
            System.arraycopy(temp[i], 0, matrixBoard[i + 1], 0, temp[i].length);
        }
    }
